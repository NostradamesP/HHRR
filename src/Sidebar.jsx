...
export default function Sidebar({ open, onClose, onQuickAction, users: allUsers = [] }) {
  const { boards, activeBoardId, switchBoard, createBoard, deleteBoard, addMember, removeMember } =
    useBoard();
  ...
  const nonMembers = allUsers.filter(
    (u) => !manageBoard?.members?.includes(u.id) && u.id !== "local-demo-user",
  );
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-xs flex-col border-l border-slate-200 bg-white shadow-2xl transition-all duration-300 sm:max-w-sm md:w-96 ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="shrink-0 border-b border-slate-200 p-4">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-red-600 text-sm font-black text-white shadow-sm">
                {APP_MONOGRAM}
              </span>
              <div>
                <h2 className="text-sm font-black text-slate-950">{APP_NAME}</h2>
                <p className="text-xs font-medium text-slate-400">
                  {appBoards.length} boards activos
                </p>
              </div>
            </div>
            <button
          ...
          {appIsAdmin && (
            <button
              onClick={() => setNewName(newName || "Nuevo board")}
              className="mb-3 flex w-full items-center gap-3 rounded-xl bg-cyan-50 px-3 py-3 text-sm font-black text-cyan-700"
            >
          ...
          <nav className="space-y-1">
            {[
              ...
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50"
              >
        ...
        <div className="flex min-h-0 flex-1 flex-col">
                ...
                <div key={b.id} className="group relative">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${boardColors[i % boardColors.length].soft}`}
                      >
                        <span
                          className={`inline-block h-2.5 w-2.5 rounded-full ${boardColors[i % boardColors.length].dot}`}
                        />
                      </span>
                      <span className="truncate">{displayBoardName(b.name)}</span>
                      <div className="ml-auto flex items-center gap-1">
                        {b.ownerId === appUser?.uid && (
                          <span className="text-[8px] font-black uppercase text-amber-500">
                            Owner
                          </span>
                        )}
                        {Array.isArray(b.members) && (
                          <span className="text-[10px] text-slate-400">{b.members.length}</span>
                        )}
                        {b.id === appActiveBoardId && (
          ...
          <div className="shrink-0 border-t border-slate-200 p-3">
            <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-2">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-cyan-600 text-xs font-black text-white">
                {(appUserData?.name || appUser?.email || "?").charAt(0).toUpperCase()}
              </span>
              <div className="min-w-0 flex-1">
                ...
                {appUserData?.jobTitle && (
                  <p className="truncate text-[10px] font-medium text-cyan-600">
                    {appUserData.jobTitle}
                  </p>
                )}
              </div>
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-sm font-black text-slate-900">
                  Miembros: {displayBoardName(manageBoard.name)}
                </h3>
                <button
                  onClick={() => setManageBoardId(null)}
              ...
              <div className="space-y-2">
                {allUsers
                  .filter((u) => manageBoard.members?.includes(u.id))
                  .map((u) => (
                    <div
                      key={u.id}
                    ...
                    >
                      ...
                      {manageBoard.ownerId === appUser?.uid && u.id !== appUser?.uid && (
                        <button
                          onClick={() => {
                            removeMember(manageBoard.id, u.id);
                          }}
                          className="flex h-7 items-center gap-1 rounded-lg border border-red-200 px-2 text-[10px] font-medium text-red-600 hover:bg-red-50"
                        >
                          <UserMinus className="h-3 w-3" /> Remover
                        </button>
                      )}
                      {u.id === manageBoard.ownerId && (
              ...
              {manageBoard.ownerId === appUser?.uid && nonMembers.length > 0 && (
                <div className="mt-4 border-t border-slate-200 pt-4">
                  <h4 className="mb-2 text-[11px] font-black uppercase text-slate-400">
                  ...
                  <div className="space-y-2">
                    {nonMembers.map((u) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            addMember(manageBoard.id, u.id);
                          }}
                          className="flex h-7 items-center gap-1 rounded-lg border border-cyan-200 px-2 text-[10px] font-medium text-cyan-700 hover:bg-cyan-50"
                        >
